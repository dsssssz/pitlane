...
- Тап по канвасу во время заезда → машина сразу встаёт на место. Тени (shadow map), отражение (Reflector) и контактная тень-диск едут вместе с машиной; номера PITLANE, покраска и мат/глянец — дети корня модели, едут сами.
...
- Отладка: `window.__pitlane3d.driveInReplay()`, `.driveInSeek(ms)` (заморозить на кадре, `null` — живое время), `.driveInFrame(ms)` (синхронный рендер → dataURL, для записи видео), `.driveInActive()`.
