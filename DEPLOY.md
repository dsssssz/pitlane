...
## 22. Заезд G63 на подиум + меню выбора машины (SW v78)

### 22.1 Drive-in (пока только G63, включается флагом)
- Флаг в каталоге: `MODEL_CATALOG[].driveIn: true` (сейчас только `g63`). Подсказки по узлам модели — `DRIVE_IN_RIGS[<id>]`: `wheels` (регэксп имён колёс), `unsprung` (суппорты — остаются ровными при клевке), `headMats` / `tailMats` (материалы фар/фонарей). Имена узлов — после санитизации GLTFLoader (пробел → `_`). Нет записи — колёса ищутся по имени (`wheel` + front/rear). Для другой машины: добавить `driveIn: true` и, при необходимости, запись в `DRIVE_IN_RIGS`.
- Когда играет: выбор машины стрелками ←→, из меню, и при открытии Бокса, если последняя машина — G63 (Бокс теперь открывается с последней машиной подиума `state.carId`, если у неё есть GLB; иначе M2, как раньше). Повторная загрузка той же модели (сброс) — без заезда. `prefers-reduced-motion` — без заезда.
- Движение: старт в 3,0 м позади точки стоянки (вдоль оси машины, со стороны тени), 1,35 с ease-out cubic; клевок при торможении до 0,021 рад (~1,2°) + просадка кузова 1,4 см, после остановки — затухающее покачивание 0,52 с. Колёса и суппорты не наклоняются вместе с кузовом (компенсирующая матрица).
- Колёса G63: в GLB 4 отдельных узла `3DWheel_Front_L/R`, `3DWheel_Rear_L/R` (Sketchfab/Forza-рип) → вращаются пропорционально пройденному пути (угол = путь / радиус; радиус и центр — по bbox колеса, ось — поперечная ось машины, считается по центрам колёс). Суппорты `Calliper_*` не вращаются.
- Свет: фары = материал `…LightA_Material1` (весь блок фар+поворотники, эмиссив холодный белый ×0,42), фонари = `red_glass` (красный ×1,6). Только цвет/интенсивность emissive (без смены шейдера → нет компиляции на старте); по окончании возвращаются точные исходные значения.
- Звук: **не** играем. Единственный ассет `audio/huracan-start.mp3` — запуск V10 Lamborghini (не G63), а переключение машин частое — будет раздражать; при открытии Бокса браузер всё равно заблокирует автозвук.
- ~~Тап по канвасу → машина встаёт на место~~ — убрано в v79 (см. 22.3). Тени (shadow map), отражение (Reflector) и контактная тень-диск едут вместе с машиной; номера PITLANE, покраска и мат/глянец — дети корня модели, едут сами.
- Тиры: high — всё; medium — тень перерисовывается каждый 2-й кадр, зеркало принудительно обновляется; low — только движение + вращение колёс (без клевка/покачивания/света). Рендер во время заезда (~1,9 с) непрерывный, потом обычный on-demand.
- Финальная поза восстанавливается из сохранённых значений `fitGlb` (position/rotation/scale корня, исходные матрицы колёс, emissive, позиция диска-тени) → idle-кадр **пиксель-в-пиксель** как у v77 (проверено: high, 390×844, 0 отличающихся пикселей).
- Отладка: `window.__pitlane3d.driveInReplay()`, `.driveInState()` (t0, t, controlsEnabled), `.driveInSeek(ms)` (заморозить на кадре, `null` — живое время), `.driveInFrame(ms)` (синхронный рендер → dataURL, для записи видео), `.driveInActive()`.

### 22.2 Меню выбора машины
- Тап по названию машины или по иконке-сетке рядом → нижняя шторка «Гараж» (стиль `race-sheet`): сетка 2 колонки (3 на ширине ≥ 440 px), превью + название + год, текущая — неоновая обводка. Тап — загрузка машины (G63 — с заездом) и закрытие. Стрелки ←→ работают как раньше. Выбор идёт через `loadPodiumModel` → `state.carId` / `applyCarUI` (синхронизация с Замером и остальным UI как у стрелок).
- Закрытие: ×, тап по фону, свайп вниз (за шапку или когда список прокручен в начало), Esc, Telegram BackButton (шторка добавлена в список `SHEETS`).
- Превью статичные: `img/cars/<id>.webp` 256×256 (~6–10 КБ), рендер настоящего подиума офлайн: `ROOT=$PWD node tools/car-thumbs.mjs` (headless Chrome + puppeteer-core; `ONLY=g63,m4` — выборочно). В precache SW v78. Новая машина в каталоге → перегенерировать превью.

### 22.3 v79: камера заблокирована во время заезда (SW v79)
- Жалоба: если крутить/зумить камеру во время заезда, машина «телепортировалась» (срабатывал тап-снап + инерция камеры).
- Тап-снап **удалён**: касание больше никогда не ставит машину на место.
- На старте заезда: текущий жест OrbitControls сбрасывается (pointer capture снимается, `end` отправляется), инерция обнуляется (`_sphericalDelta`, `_panOffset`, `_scale`), `controls.enabled = false` → нет вращения, зума колесом, pinch, pan. `controls.update()` на время заезда не вызывается → камера неподвижна (auto-rotate/демпфирование не двигают её).
- По окончании — повторный сброс накопленного и `controls.enabled` возвращается к прежнему значению. То же при прерывании: смена машины (`fitGlb`), уход с вкладки Бокс (`goToView`), скрытие вкладки браузера, канвас вне экрана.
- Проверка (headless Chrome, SwiftShader): флик + незавершённый drag в момент старта, затем во время заезда drag / колесо / pinch (CDP touch) / тапы → камера во всех кадрах заезда не менялась, `controls.enabled=false` в каждом кадре, положение машины в каждом кадре совпадает с кривой ease-out (ошибка 0,0000 м, без скачков), первый кадр после заезда — та же камера (нет отложенной инерции); после — drag вращает, колесо зумит; прерывание сменой машины/вкладкой возвращает управление. Idle G63 high — 0 отличающихся пикселей от v77.

## 23. Обратная связь + аудит безопасности (SW v80)

### 23.1 Обратная связь
- Профиль → кнопка **«Обратная связь»** → шторка (стиль `race-sheet`, в списке TMA `SHEETS` → BackButton закрывает). Тип: Ошибка / Идея / Жалоба / Другое; текст 10–2000 символов со счётчиком; контакт (необязательно); скриншот (необязательно) — на клиенте JPEG, длинная сторона ≤ 1280 px, ≤ 400 КБ (качество/размер понижаются ступенями; SVG не принимается). Работает и без входа.
- Автоматически прикладывается: версия приложения (`APP_VERSION`), кэш SW (`pitlane-vNN`), User-Agent, тир 3D, TMA да/нет (+ платформа Telegram), текущая вкладка, язык, размер экрана. **Не** передаются: координаты GPS, телефон (сервер дополнительно пропускает только белый список полей).
- Успех → тост «Спасибо! Мы прочитаем». Нет сети → сообщение кладётся в очередь `localStorage['pitlane-feedback-queue-v1']` (до 3 шт., 7 дней) и отправляется при событии `online` / через 8 с после следующего запуска; тост «Нет сети — отправим, когда появится интернет». HTTP-ошибки (429, большой скриншот) показываются в шторке.
- Worker `POST /feedback`: тело ≤ 720 КБ; honeypot-поле `website` + минимум 3 с на форме (боты получают фиктивный 200, ничего не сохраняется); лимиты: IP 5/час, пилот (аккаунт или гостевой device id) 10/сутки, всего 300/сутки. Хранение: KV `feedback:<ms>:<id>` (TTL 180 дней, metadata `{pid,type,shot}`; при удалении аккаунта его отзывы удаляются).
- Пересылка владельцу: Bot API `sendMessage` (plain text, без `parse_mode` → пользовательский текст не может внедрить разметку) + `sendPhoto` со скриншотом, токен — секрет `TELEGRAM_BOT_TOKEN`, чат — var `FEEDBACK_CHAT_ID` в `worker/wrangler.toml` (пусто → только KV). Ошибка Telegram не теряет отзыв (сохранён, ответ 200, `forwarded:false`).

**Как получить `FEEDBACK_CHAT_ID` (делает Мага, 2 минуты):**
1. В Telegram открыть @pitlane_official_bot и отправить `/start` (иначе бот не сможет писать вам).
2. Узнать свой chat id (у бота нет вебхука → работает `getUpdates`):
   `TELEGRAM_BOT_TOKEN='<токен из BotFather>' node worker/scripts/feedback-chat-id.mjs`
   или в браузере `https://api.telegram.org/bot<ТОКЕН>/getUpdates` → `result[].message.chat.id` (число; для группы — отрицательное).
   Отдельного admin-эндпоинта для этого нет намеренно.
3. Вписать в `worker/wrangler.toml` → `[vars] FEEDBACK_CHAT_ID = "123456789"` (и в корневой `wrangler.toml`, он дублирует конфиг) и задеплоить: `cd worker && npx wrangler@3.114.17 deploy`. Не заводить его как secret с тем же именем (конфликт с var).
- Прочитать отзывы без Telegram: `cd worker && npx wrangler@3.114.17 kv key list --namespace-id fc7e076d77824da184e7314c2fb3ff1b --prefix feedback:` и `kv key get --namespace-id … "feedback:…"`.

### 23.2 Безопасность (подробности — отчёт `pitlane-security-audit.md`)
- Worker: заголовки `nosniff`, `X-Frame-Options: DENY`, `CSP default-src 'none'`, HSTS, `Cache-Control: no-store`; CORS только `https://dsssssz.github.io` (localhost — через `EXTRA_ORIGINS` локально); лимит тела на каждом маршруте (8 КБ share … 1,6 МБ garage, иначе 413); 500 без текста ошибки/стека.
- Лимиты: биндинги Workers Rate Limiting `RL_READ` 240/мин и `RL_WRITE` 40/мин на IP (в памяти, без затрат KV) + окна в KV на каждую запись (tops 60/ч на пилота, pulse 10/ч и 40/сут, share 30/ч на IP + 3000/сут всего, дуэль/экипаж 20/10 в час, вступление по коду 20/ч, check-in 10/ч, garage 60/ч, удаление аккаунта 3/ч, OTP-verify 30/15 мин, SMS 300/сут всего). Счётчик KV больше не пишет, когда лимит уже превышен, и окно фиксированное.
- IDOR: `/crews?mine=` и `/duels?mine=` отдают только свой список; код приглашения виден только участникам; гостевые device id публикуются лишь как `g_<sha256>` (клиент распознаёт себя через `isMyPilotId`); гость без device id (`guest:<ник>`) больше не может создавать/играть дуэли и экипажи; id поста Pulse генерирует сервер.
- Валидация: круг 15 с…60 мин, секунды < 60, `ms` согласован с `t` (±1 с), секторы ≤ времени круга; 0–100 от 1,5 до 60 с; NaN/отрицательные/абсурдные `dist/slipAvg/avgAcc/hz` отбрасываются; id трасс/машин — только slug (`[A-Za-z0-9_.-]{1,64}`); `<>`/управляющие символы вырезаются из ников, машин, названий экипажей, заметок; share-карточки — белый список полей (и при чтении старых); аватары-URL только с CDN Telegram.
